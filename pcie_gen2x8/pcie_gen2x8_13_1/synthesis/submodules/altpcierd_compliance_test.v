...
//
assign exit_ltssm_compliance = ltssm_cnt_cycles[6];
always @(posedge refclk or negedge rstn) begin
   if (rstn == 1'b0) begin
      ltssm_cnt_cycles     <= 7'h00;
   end
   else begin
      if (new_edge_for_compliance==1'b1) begin
         ltssm_cnt_cycles <= 7'h7f;
      end
      else if (ltssm_cnt_cycles != 7'h00) begin
         ltssm_cnt_cycles <= ltssm_cnt_cycles - 7'h01;
      end
   end
end
